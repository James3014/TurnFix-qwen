...
def test_full_system_workflow(test_client):
    """測試完整系統工作流程"""
    # 1. 創建症狀
...
def test_symptom_management_workflow(test_client):
    """測試症狀管理完整工作流程"""
    # 創建症狀
    ...
    assert update_data["symptom"]["name"] == "更新後的換刃困難_測試"
    
    # 刪除症狀
    delete_response = test_client.delete(f"/api/v1/admin/symptoms/{symptom_id}")
    assert delete_response.status_code == 200

def test_practice_card_management_workflow(test_client):
    """測試練習卡管理完整工作流程"""
    # 創建練習卡
    create_response = test_client.post(
        "/api/v1/admin/practice-cards",
    ...
    get_data = get_response.json()
    assert get_data["status"] == "success"
    assert get_data["practice_card"]["name"] == "重心轉移練習_測試"
    ...
    update_data = update_response.json()
    assert update_data["status"] == "success"
    assert update_data["practice_card"]["name"] == "更新後的重心轉移練習_測試"
    
    # 刪除練習卡
    delete_response = test_client.delete(f"/api/v1/admin/practice-cards/{practice_card_id}")
    assert delete_response.status_code == 200

def test_symptom_practice_mapping_workflow(test_client):
    """測試症狀練習卡映射管理完整工作流程"""
    # 創建症狀
    symptom_response = test_client.post(
        "/api/v1/admin/symptoms",
...
def test_feedback_system_workflow(test_client):
    """測試回饋系統完整工作流程"""
    # 創建會話回饋
    session_feedback_response = test_client.post(
        "/api/v1/session-feedback",
    ...
    # 更新最愛狀態
    if "feedback" in practice_card_feedback_data:
        feedback_id = practice_card_feedback_data["feedback"]["id"]
        update_favorite_response = test_client.put(
            f"/api/v1/practice-card-feedback/{feedback_id}/favorite",
            json={"is_favorite": True}
        )
        assert update_favorite_response.status_code == 200

def test_feedback_analytics_workflow(test_client):
    """測試回饋分析完整工作流程"""
    # 獲取回饋分析摘要
    ...
    assert "total_session_count" in summary_data
    assert "rating_distribution" in summary_data
    assert "average_rating" in summary_data
    assert "rating_count" in summary_data
    assert "favorite_count" in summary_data
    assert "favorite_rate" in summary_data
    assert "total_feedback_count" in summary_data
    ...
    assert "card_info" in card_analysis_data
    assert "rating_distribution" in card_analysis_data
    assert "average_rating" in card_analysis_data
    assert "rating_count" in card_analysis_data
    assert "favorite_count" in card_analysis_data
    assert "favorite_rate" in card_analysis_data
    
    # 獲取症狀回饋分析
    symptom_analysis_response = test_client.get("/api/v1/admin/feedback-analytics/symptoms/1")
    assert symptom_analysis_response.status_code in [200, 404]  # 可能返回404如果症狀不存在
    
    # 獲取用戶偏好分析
    user_preference_response = test_client.get("/api/v1/admin/feedback-analytics/user-preferences")
    assert user_preference_response.status_code in [200, 404]  # 可能返回404如果用戶偏好不存在